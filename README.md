...
When the page is compiled, copy the 4 files directly to the project root directory. `vite.config.js` needs to add the following configuration. You can refer to [demo vite.config.js](https://github.com/primus-labs/zktls-demo/blob/main/page-core-sdk-example/vite.config.js).
...
The Production Example and Test Example processes are the same. The difference is that the appSecret is stored on your server, and when signing the attestation request parameters, the parameters are passed to your server, which signs them and then passes them to the frontend.
...
Frontend does not require appSecret parameter to initialize SDK.
...
The server is mainly responsible for obtaining the attestation parameters generated by the frontend, and then using appSecret to sign the proof parameters.
